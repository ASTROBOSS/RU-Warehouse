...
public class Warehouse {
    ...
    public void betterAddProduct(int id, String name, int stock, int day, int demand) {
        ...
        if(sectors[id % 10].getSize() == 5) {
            for(int i = 1; i < 10; i++) {
                if(sectors[(id % 10 + i) % 10].getSize() < 5) {
                    sectors[(id % 10 + i) % 10].add(new Product(id, name, stock, day, demand));
                    sectors[(id % 10 + i) % 10].swim(sectors[(id % 10 + i) % 10].getSize());
                    return;
                }
            }
        }
